...
| `interrupted` | last record's `stop_reason=null` AND not `clean-success` AND iter count < MaxIterations; OR a stale-running sentinel (state `running`, pid not a live ilk process) would otherwise have claimed `clean-success` | Window was killed externally (chad ran `stop.ps1` or closed window), or the runner died before `Finalize-Sentinel` — either way the loop did not reach a natural stop. Clean the stale sentinel before relaunching. |
...
| `local-checks-broken` | a `local_checks` result where the gate COMMAND could not execute — exit_code in {4, 5, 127} or stderr matches a "couldn't execute" pattern (`_is_broken_gate_result`) | The gate toolchain itself is broken (missing module, command not found, collection error). The product code is NOT implicated; a blind resume re-fails identically. Fix the gate config/environment before relaunching. |
...
| `dependency-unreachable` | `stop_reason="no-progress"` AND iteration logs show a missing MCP / unreachable `env_prereq` (detected by `detect_unreachable_dependency`) | A required dependency (MCP server, external service) was unreachable. A restart won't help; fix the config/reachability (e.g. `ilk-worker-mcp add <name>`). |
| `model-incapability` | `stop_reason="no-progress"` AND logs initially match `dependency-unreachable` but the named MCP's tool calls succeeded (e.g. text-only model receiving an image) | The stall is a model capability gap, not a connectivity issue. The dependency IS reachable; the model cannot process the input format. Switch model or adjust the task. |
