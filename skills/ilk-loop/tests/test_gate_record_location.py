...
def _git(project: Path, *args: str) -> str:
    return subprocess.run(
        ...
        cwd=project, capture_output=True, text=True, encoding="utf-8", errors="replace", text=True, check=True,
    ).stdout.strip()
...
class TestD5OneRecordLocation:
    ...
    def test_runner_does_not_send_the_gate_to_the_launcher_dir(
        ...
        subprocess.run(["bash", "-c", script], capture_output=True, text=True, encoding="utf-8", errors="replace", text=True,
