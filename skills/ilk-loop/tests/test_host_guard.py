...
class TestGuardLetsLocalBinariesThrough:
    ...
    def test_git_is_allowed(self) -> None:
        out = subprocess.run(["git", "--version"], capture_output=True, text=True, encoding="utf-8", errors="replace", text=True)
        assert out.returncode == 0, "git must pass through the guard untouched"

    def test_bash_is_allowed(self) -> None:
        out = subprocess.run(["bash", "-c", "echo ok"], capture_output=True, text=True, encoding="utf-8", errors="replace", text=True)
...
class TestMarkerExemption:
    ...
    def test_marker_stands_guard_down(self) -> None:
        ...
        out = subprocess.run(["launchctl", "list"], capture_output=True, text=True, encoding="utf-8", errors="replace", text=True)
...
class TestReportOnlyMode:
    ...
    def test_report_only_allows_launchctl(self, monkeypatch, exempts_recorded_during) -> None:
        ...
        result = subprocess.run(["launchctl", "list"], capture_output=True, text=True, encoding="utf-8", errors="replace", text=True, timeout=5)
