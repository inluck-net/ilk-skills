...
def test_single_mode_unchanged(tmp_path: Path) -> None:
...
def test_meta_mode_basic(tmp_path: Path) -> None:
    print("test_meta_mode_basic:")
    meta = tmp_path / "umbrella"
    meta.mkdir()
...
def test_meta_with_worktree_member(tmp_path: Path) -> None:
...
def test_invalid_marker_rejected(tmp_path: Path) -> None:
...
def test_invalid_outer_does_not_block_valid_inner(tmp_path: Path) -> None:
...
def test_no_project_anywhere(tmp_path: Path) -> None:
...
def test_plans_dir_uses_meta_key(tmp_path: Path) -> None:
    ...
    sandbox = tmp_path / "ilk-data-sandbox"
