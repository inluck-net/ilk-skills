...
def _run_scheduler(sandbox: _SchedulerSandbox, *, cwd: Path | None = None,
                   extra_env: dict | None = None,
                   timeout: int = 30) -> subprocess.CompletedProcess:
    """Run scheduler.sh --once --dry-run under the sandbox env."""
    env = {**sandbox.env}
    if extra_env:
...
def _run_scheduler_raw(home: Path, *, cwd: Path | None = None,
                       extra_env: dict | None = None,
                       timeout: int = 30) -> subprocess.CompletedProcess:
    """Run scheduler.sh --once --dry-run with raw HOME (no fixture).

    Used by tests that need .ilk-data to be absent at the start — the
    scheduler_sandbox fixture eagerly creates it, so these tests carry
    ``@pytest.mark.allow_real_data_home`` instead.
    """
    env = {
        **os.environ,
        "HOME": str(home),
...
def test_writes_state_file_with_required_fields(tmp_path, scheduler_sandbox):
    """AC-1: scheduler.state.json must contain pid, started_at, toolkit_head."""
    _run_scheduler(scheduler_sandbox)
...
def test_pid_field_is_integer(tmp_path, scheduler_sandbox):
    """AC-1: pid must be an integer matching the scheduler's own PID."""
    res = _run_scheduler(scheduler_sandbox)
    state_file = tmp_path / ".ilk-data" / "scheduler.state.json"
    assert state_file.is_file()
    data = json.loads(state_file.read_text(encoding="utf-8"))
...
def test_started_at_is_iso8601(tmp_path, scheduler_sandbox):
...
def test_toolkit_head_matches_repo_head(tmp_path, scheduler_sandbox):
...
def test_toolkit_head_ignores_pwd(tmp_path, scheduler_sandbox):
    """AC-2: toolkit_head must be correct even when $PWD is unrelated."""
    unrelated = tmp_path / "unrelated_dir"
    unrelated.mkdir()
    _run_scheduler(scheduler_sandbox, cwd=unrelated)
    state_file = tmp_path / ".ilk-data" / "scheduler.state.json"
    assert state_file.is_file()
    data = json.loads(state_file.read_text(encoding="utf-8"))
    expected = _expected_toolkit_head()
...
def test_toolkit_head_is_40char_hex(tmp_path, scheduler_sandbox):
...
def test_bare_pid(tmp_path, scheduler_sandbox):
...
def test_rewrite_on_restart(tmp_path, scheduler_sandbox):
...
def test_fail_open_on_readonly_dir(tmp_path, scheduler_sandbox):
    ...
    try:
        res = _run_scheduler(scheduler_sandbox, timeout=30)
...
def test_fail_open_logs_degradation(tmp_path, scheduler_sandbox):
...
@pytest.mark.allow_real_data_home  # fixture eagerly creates .ilk-data/logs/, breaking the precondition
