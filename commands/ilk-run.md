...
Claude** on a separate Anthropic-compatible provider, there are two paths:

**Preferred — the `claude-worker` launcher engine.** Set the engine and the
launcher routes the detached run under the worker home for you (injects
`CLAUDE_CONFIG_DIR=~/.claude-worker` + `ILK_SKILL_HOME`), with the same
fail-closed settings preflight:
...
Or persist it per-project so the watchdog/scheduler reuse it:
...
**Manual — the worker wrapper.** Launch the runner through the wrapper so it
inherits the worker home directly (useful for interactive one-offs):
